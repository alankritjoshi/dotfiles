...
assert("git diff", isSafeCommand("git diff"), true);
assert("git show", isSafeCommand("git show HEAD"), true);
assert("gt ls stack", isSafeCommand("gt ls --stack"), true);
assert("gt log", isSafeCommand("gt log short"), true);
assert("gt trunk", isSafeCommand("gt trunk"), true);
assert("gt branch info", isSafeCommand("gt branch info"), true);
...
assert("git add", isSafeCommand("git add file.txt"), false);
assert("git switch create", isSafeCommand("git switch -c new-branch"), false);
assert("git checkout create", isSafeCommand("git checkout -b new-branch"), false);
assert("git checkout existing", isSafeCommand("git checkout existing-branch"), false);
assert("git branch delete", isSafeCommand("git branch -d old-branch"), false);
assert("gt checkout", isSafeCommand("gt checkout branch"), false);
assert("gt co", isSafeCommand("gt co branch"), false);
assert("gt top", isSafeCommand("gt top"), false);
assert("gt up", isSafeCommand("gt up"), false);
assert("gt modify", isSafeCommand("gt modify -a -m 'test'"), false);
assert("gt submit", isSafeCommand("gt submit --no-interactive"), false);
assert("gt continue", isSafeCommand("gt continue"), false);
